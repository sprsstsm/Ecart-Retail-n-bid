...
$blogid=$_GET["blog_id"];
 ...
 {
 $views=mysqli_fetch_array(mysqli_query($con,"SELECT  views FROM   blog_views  where blogid=$blogid AND userid=$_SESSION[user_id]"));
//echo "aaaaaaaaaaaaaa";print_r($views['views']);
   //$count=$views['views'];
   //$count= $count+1;
...
{
     mysqli_query($con,"insert into   blog_views(blogid,userid,views) values($blogid,$_SESSION[user_id],1)");    
}
...
if(isset($_SESSION["server_detail"]))
{
	$blogid=$_GET["blog_id"];
	//$views=mysqli_fetch_array(mysqli_query($con,"SELECT  views FROM   blog_views  where blogid=$blogid AND serverid=$_SESSION[server_detail]"));

			$sid=$_SESSION['server_detail'];
    mysqli_query($con,"insert into   blog_views(blogid,userid,views,serverid) values($blogid,0,1,$sid)");     
		
	if (time() - $_SESSION['CREATED'] > 1) {
  unset($_SESSION['blogid']); 
	}
}
 $curblog = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM blog WHERE blog_id='".$_REQUEST['blog_id']."'")); 
?>                    
                            <div id="html-section-blog-template" class="html-section">
                                ...
                                <?php 
                                $Cnt = mysqli_num_rows(mysqli_query($con,"SELECT `cmd_id` FROM `blog_cmd` WHERE `blog_id`='".$_REQUEST['blog_id']."'"));

                                $views_count=mysqli_fetch_array(mysqli_query($con,"SELECT count(blogid) as cob FROM `blog_views` WHERE `blogid`='".$_REQUEST['blog_id']."'"));
                                ?>
                                <span class="comment"><?php echo $Cnt." Comments"; ?> </span> <span class="comment"> <?php if(!empty($_SESSION['user_id'])) { ?>  TOTAL VIEWS-<?php print_r($views_count['cob']);  ?>  <?php } ?></span>
                                                                            </p>
                                                                    </div>
    ...
    <?php 
      $abc = mysqli_query($con,"SELECT `cmd_id`, `blog_id`, `user_id`, `blogcmd_name`, `blogcmd_email`, `blogcmd_message`, `cteate_on` FROM `blog_cmd` WHERE `blog_id`='".$_REQUEST['blog_id']."'");
      while ($cmd= mysqli_fetch_assoc($abc)){

        $user = mysqli_fetch_assoc(mysqli_query($con,"SELECT `id`, `username`, `useremail`, `userphone`, `userimage`, `password`, `business`, `professional`, `create_on`, `update_on` FROM `user` WHERE `id`='".$cmd['user_id']."'"));
    ?>
        <li>
     ...
     <?php } ?>
        ...
        padding-top: 10px;    margin-top: 8%;">
            ...
            <?php 
                $blog_col = mysqli_query($con,"SELECT * FROM blog WHERE admin_approval='yes' ORDER BY blog_id DESC LIMIT 4"); 
...
<?php include('Modal.php');?>
  ...
  <script type="text/javascript">
      function addComment(){
        ...
        var blog_id = "<?php echo $_REQUEST['blog_id'];?>";
