...
<?php 
        ...
        $blog = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `blog` WHERE `blog_id` = '$blog_id'"));

        $TagcontentAr=explode(',',$blog ['tag']);
        $combine_json = json_encode($TagcontentAr);
?>
<?php 
   if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // }{
            $video_name = $_POST['hidvideo'];
        //}

        $query = "UPDATE `blog` SET `category`='$blog_category',`title`='$blog_title',`content`='$blog_content',`image`='$file_name',`user`='$user',`cat_id`='$blog_category_id',`short_desc`='$short_desc',`video`='$video_name',`tag`='$TagContent' WHERE `blog_id`='$blog_id'";
...
.btnContactSubmit
{
    ...
    <body id="digital-world-store" class="template-index" >
        ...
        <div id="PageContainer" class="is-moved-by-drawer">
            ...
            <form href="update_blog.php" id="BlogUpload" name="BlogUpload" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="hidid" value="<?php echo $_REQUEST['blog_id']; ?>">
                <input type="hidden" name="hidfile" value="<?php echo $blog['image']; ?>">
                <input type="hidden" name="hidvideo" value="<?php echo $blog['video']; ?>">
               ...
               <div class="row">
                 ...
                 <select class="form-control" placeholder="Blog Category *"  id="blogger_name" name="blogger_name" >
                    ...
                    <?php echo ($blog['user'] == $username) ? 'selected' : '';?> ><?php echo $username; ?></option>
                     <option value="JLODA team"
                     <?php echo ($blog['user'] == 'JLODA team' ) ? 'selected' : '';?> >JLODA team</option>
                     <option value="unknown" 
                     <?php echo ($blog['user'] == 'unknown' ) ? 'selected' : '';?> >unknown</option>
                 </select>
                 ...
                 <select class="form-control" placeholder="Blog Category *"  id="blog_category" name="blog_category" >
                     ...
                     ?>
                        <option value="<?php echo $row['cat_id'].','.$row['category']?>"
                        <?php echo ($blog['cat_id'] == $row['cat_id']) ? 'selected' : '' ;?> ><?php echo $row['category']?> </option>
                     <?php } ?>
                 </select>
                 <p id="Blogerr1" class="ERROR"></p>
                        ...
                        <div class="form-group">
                 <input type="text"  class="form-control" placeholder="Blog Title *"  id="blog_title" name="blog_title" value="<?php echo $blog['title']; ?>" >
                 <p id="Blogerr2" class="ERROR"></p>
                        </div>
                        <div class="form-group">
                            <img src="Blogimage/<?php echo $blog['image']; ?>" style="height: 200px;width: 200px;margin: 0% 33%;" >
                        ...
                        <div class="form-group">
                <textarea class="form-control" type="file" id="short_desc" name="short_desc" placeholder="Short Description"><?php echo  $blog['short_desc']; ?></textarea>
                        ...
                        <div class="form-group">
                            <input type="button" onclick="addBlog()" name="Add_Blog" class="btnContact" value="Update Blog">
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    var Tagcontentar = <?php global $combine_json; echo $combine_json;  ?>;

    $.each(Tagcontentar, function( index, tag ) {

        $('#dynamic_data').append('<div class="col-sm-8 col-sm-offset-2 appendData" style="margin-top: 5px;"><div class="col-md-10" style="float:left" ><input type="text" class="form-control1" name="TagContent[]" value="'+tag+'" title="Tag Contents"></div><div class="col-md-2" style="padding: 8px;float:left"><i class="fa fa-trash-o" onclick="delThis(event)"></i></div></div>');
